...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (process.env.ALLOWED_HOSTNAMES) {
      allowedHostnames = process.env.ALLOWED_HOSTNAMES.split(',');
    };

    if (allowedHostnames.indexOf(location.hostname) === -1 ||
       (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname))) {
